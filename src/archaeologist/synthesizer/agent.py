...
def synthesize_narrative(
    ...
    if est_tokens <= MAX_SINGLE_CALL_TOKENS:
        if on_progress:
            on_progress({"step": "single_call", "detail": "Synthesizing full narrative..."})
        return _single_call_synthesis(serialized, total_chunks, model)
    else:
        return _hierarchical_synthesis(extractions, model, on_progress)
...
def _hierarchical_synthesis(
    ...
    for batch_start in range(0, len(extractions), BATCH_SIZE):
        ...
        if on_progress:
            on_progress({"step": "batch", "batch": batch_num, "total_batches": total_batches})
    ...
    if on_progress:
        on_progress({"step": "merge", "detail": f"Merging {len(section_narratives)} sections..."})
